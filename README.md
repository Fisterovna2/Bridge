...
4. Logs appear in **Logs / Dev** tab, and are saved to `logs/session/session.jsonl`.
...
Policies are applied per mode, with rule IDs logged for each decision.
```
models:
  default:
    vision: mvp
...
5. Install the **VirtualBox Extension Pack** if you need VRDE/RDP.
6. (Optional) Enable **VRDE** (RDP) for debugging:  
...
### Game loop (VM-only)
1. Select a game profile in **Modes**.
2. Click **Start Game Loop (VM)** to run the capture→plan→input loop.
3. Click **Stop Game Loop** to stop safely.
...
- Session logs: `logs/session/session.jsonl` (structured, no raw OCR/PII).
- Session recordings: `logs/session/frames/*.png` and `logs/session/actions.jsonl`.
- Replay (dry-run, ghost cursor only):
  ```bash
...
## Troubleshooting
- **PySide6 / GUI errors**: install PySide6 and ensure a GUI environment is available.
- **Headless CI**: optional GUI/host input deps are lazy-loaded to avoid import crashes.
- **DISPLAY/libEGL**: run GUI tools on a machine with display or use WSLg.
