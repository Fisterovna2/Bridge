...
def test_policy_denies_host_ops_outside_normal() -> None:
    engine = PolicyEngine()
    decision = engine.evaluate(RunMode.GAME, "open C:\\Windows\\system32", ["Documents"])
    assert decision.allowed is False
    assert decision.rule_id == "deny-host-ops-outside-normal"
