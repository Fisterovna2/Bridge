...
from ai_bridge.core.safety import (
    RiskLevel,
    score_risk,
    allowlist_ok,
    destructive_match,
...
class PolicyEngine:
    def evaluate(self, mode: RunMode, text: str, allowlist_paths: list[str]) -> PolicyDecision:
        ...
        if mode != RunMode.NORMAL and (looks_like_path(lowered) or process_control_match(lowered)):
            return PolicyDecision(
                allowed=False,
                requires_confirmation=True,
                reason="Host filesystem/process control denied outside Normal mode",
                rule_id="deny-host-ops-outside-normal",
                risk=RiskLevel.HIGH,
                target=target,
                mode=mode,
            )
