...
class BridgeOrchestrator:
    def __init__(
        ...
        self.config = config or OrchestratorConfig(
            ...
            logs_path=Path("logs/session/session.jsonl"),
    ...
    def log_event(self, event: str, payload: dict) -> None:
        ...
        if self.recorder:
            summary_path = self.recorder.session_dir / "summary.json"
            summary_path.write_text(
                json.dumps({"last_hash": self._last_hash}, ensure_ascii=False, indent=2),
                encoding="utf-8",
            )
